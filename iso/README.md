...
- Credentials are baked in and **intentionally temporary**: both `archuser`
  and `root` get the password `ChangeMe-ArchProject!1`, and a `chage -d 0`
  custom command forces a password change at the very next interactive
  login. This password is deliberately public (it's committed to this repo)
  - it is only ever meant to survive from first boot to first login.
- After archinstall's silent run finishes, the ISO's script reboots the
  ...
  `ansible-playbook site.yml`, then disables itself.
...
`bootstrap/user_configuration.template.json` plus the forced-expiry
commands) via `generate-config.py`, and runs `mkarchiso`. Output lands in
`iso/out/*.iso`.
